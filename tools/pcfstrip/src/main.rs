...
use std::{collections::HashMap, env, fs::{File, OpenOptions}, io::{BufWriter, Seek}};
...
fn main() -> anyhow::Result<()> {
    let input_path = env::args().nth(1).expect("input pcf file path not given");
    let output_path = env::args().nth(2).expect("output pcf file path not given");
    let expected_size: u64 = env::args().nth(3).expect("expected size not given").parse()?;
    
    let default_system_map = get_particle_system_defaults();
    // let default_attribute_map = get_default_attribute_map()?;
    let default_attribute_map: HashMap<&str, Attribute> = HashMap::from([
    ...
    let mut file = File::open_buffered(input_path)?;
    let dmx = dmx::decode(&mut file)?;
    let pcf: Pcf = dmx.try_into()?;
    
    // whats goin on with the 431st particle system? (0-indexed)
    let pcf = pcf.defaults_stripped_nth(611, &default_system_map, &default_attribute_map);
    ...
    let vpk_path = "/home/snale/.local/share/Steam/steamapps/common/Team Fortress 2/tf/tf2_misc_dir.vpk";
    let mut vpk = vpk::from_path(vpk_path)?;
    vpk.patch_file(&output_path, reader.get_ref().len() as u64, &mut reader)?;

    // for _ in file.stream_len()?..expected_size {
    //     file.write_u8(0)?;
    // }
